    ...
    <header className="fixed m-5 z-10 ">
      <div className="flex items-center justify-between px-5 py-3">
        ...
        <div className="flex items-center">
          ...
          <a className={`bg-main-color-gray px-3 py-1 border-solid border-2 border-orange-200 shadow-custom transition ease-in-out ${hoverBtnsEffectDisabled ? 'hover:shadow-custom-hover hover:-translate-y-1 hover:scale-105' : ''}`} href="/cv-xportas.pdf" download="cv-xportas.pdf" onMouseDown={() => setHoverBtnsEffectDisabled(false)} onMouseUp={() => (setHoverBtnsEffectDisabled(true), setShowNavbar(!showNavbar))} onMouseLeave={() => setHoverBtnsEffectDisabled(true)} >
            <svg width="35px" height="25px" xmlns="http://www.w3.org/2000/svg">
              <text className="text-orange-200 font-header" x="50%" y="60%" text-anchor="middle" dominant-baseline="middle" fill="rgb(254, 215, 170)">CV</text>
            </svg>
          </a>

          <a className={`bg-main-color-gray px-3 py-1 border-solid border-2 border-orange-200 shadow-custom transition ease-in-out ${hoverBtnsEffectDisabled ? 'hover:shadow-custom-hover hover:-translate-y-1 hover:scale-105' : ''}`} onMouseDown={() => setHoverBtnsEffectDisabled(false)} onMouseUp={() => (setHoverBtnsEffectDisabled(true), setShowNavbar(!showNavbar))} onMouseLeave={() => setHoverBtnsEffectDisabled(true)} href="https://github.com/xportas">
            <svg width="25px" height="25px" viewBox="0 0 24.00 24.00" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="#fed7aa" stroke-width="0.00024000000000000003">
              <g id="SVGRepo_bgCarrier" stroke-width="0" />
        ...
        <div className="fixed left-1/2 transform -translate-x-1/2 z-10">
          <nav className={`flex bg-main-color-gray px-5 py-1 border-solid border-2 border-orange-200 shadow-custom ${showNavbar ? 'animate-fadeIn' : 'opacity-0'}`}>
            <a href="#home-section" class="px-3 "><span class="text-orange-200">// home</span></a>
            <a href="#expertise-section" class="px-3 "><span class="text-orange-200">// expertise</span></a>
            <a href="#work-section" class="px-3 "><span class="text-orange-200">// work</span></a>
            <a href="#experience-section" class="px-3 "><span class="text-orange-200">// experience</span></a>
            <a href="#contact-section" class="px-3 "><span class="text-orange-200">// contact</span></a>
          </nav>
        </div>
